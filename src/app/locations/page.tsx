...
export default function LocationsPage() {
    ...
    const loadLocations = useCallback(async () => {
        ...
        setLoading(true);
        try {
            const data = await locationService.listByProject(selProject);
    ...
    const createLocation = async () => {
        ...
        try {
            await locationService.create({ ...form, project_id: selProject });
                ...
                <div className="page-container" style={{ maxWidth: 900, margin: '0 auto' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, flexWrap: 'wrap', gap: 10 }}>
                        ...
                        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                            ...
                            <button className="btn btn-primary" onClick={() => setShowCreate(true)}>+ Add Location</button>
                        </div>
                    </div>

                    {showCreate && (
                        <div className="card" style={{ padding: 20, marginBottom: 16 }}>
                            <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 12 }}>📍 New Location</div>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(260,1fr))', gap: 12 }}>
                            {locations.map(loc => (
                                <div key={loc.id} className="card" style={{ padding: 16, position: 'relative' }}>
                                    ...
                                    <button onClick={() => deleteLocation(loc.id)} style={{ position: 'absolute', top: 10, right: 10, fontSize: 14, background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-danger)', opacity: 0.5 }}>🗑</button>
                                </div>
                            ))}
                        </div>
                    )}
