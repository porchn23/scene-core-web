...
import { projectService, sceneService, shotService, characterService, actorService, locationService } from '@/lib/services';
import type { ProjectRead, ProjectUpdate, SceneRead, SceneCreate, SceneUpdate, ShotRead, ShotCreate, ShotUpdate, CharacterRead, ActorRead, LocationRead } from '@/lib/services';
...
function ActionText({ text, characters }: { text: string; characters: CharacterRead[] }) {
    ...
    return (
        <span style={{ whiteSpace: 'pre-wrap' }}>
            {parts.map((p, i) => {
                const match = sorted.find(c => c.name.toLowerCase() === p.toLowerCase());
                if (match) return <strong key={i} style={{ color: 'var(--color-accent)', fontWeight: 800, background: 'rgba(99,102,241,0.08)', padding: '0 2px', borderRadius: 2 }}>{p.toUpperCase()}</strong>;
                return p;
            })}
        </span>
    );
...
function ShotBlock({ shot, isActive, onActivate, onSave, onDelete, onRender, scrollRef, characters, actors }: ShotBlockProps) {
    ...
    return (
        ...
        >
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 6, padding: '10px 24px 6px', borderTop: isActive ? '2px solid var(--color-accent)' : '2px solid transparent' }}>
                ...
                <div style={{ display: 'flex', gap: 5, alignItems: 'center', flexWrap: 'wrap' }}>
                    ...
                    {status === 'draft' && <button className="btn btn-primary" style={{ padding: '3px 10px', fontSize: 10 }} onClick={e => { e.stopPropagation(); onRender(); }}>▶ Render</button>}
            ...
            <div style={{ padding: '0 24px', marginBottom: 4 }}>
                ...
                {actionFocused ? (
                    <textarea
                        value={script.action}
                        onChange={e => updateScript('action', e.target.value)}
                        onBlur={() => { setActionFocused(false); save({ script }); }}
                        autoFocus
            ...
            {/* ── PRODUCTION NOTES ── */}
            <div style={{ padding: '0 24px 10px', marginBottom: 4 }}>
                <textarea
...
function LocationForm({ location, loading, onSubmit, onClose }: { location?: Partial<LocationRead>; loading: boolean; onSubmit: (d: any) => void; onClose: () => void; }) {
    ...
    const [lighting, setLighting] = useState(location?.lighting_hint ?? '');
    const [imgUrl, setImgUrl] = useState(location?.image_reference_url ?? '');
    function submit(e: React.FormEvent) { e.preventDefault(); onSubmit({ name, description: desc || null, lighting_hint: lighting || null, image_reference_url: imgUrl || null }); }
    return (
...
export default function ProjectDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const loadShotsFor = useCallback(async (sceneId: string) => {
        if (sceneShots[sceneId] || loadingShots[sceneId]) return;
        setLoadingShots(p => ({ ...p, [sceneId]: true }));
        try { const s = await shotService.listByScene(sceneId); setSceneShots(p => ({ ...p, [sceneId]: s.sort((a, b) => a.shot_order - b.shot_order) })); }
        catch { setSceneShots(p => ({ ...p, [sceneId]: [] })); }
        finally { setLoadingShots(p => ({ ...p, [sceneId]: false })); }
    }, [sceneShots, loadingShots]);
    ...
    useEffect(() => {
        (async () => {
            ...
            } catch (e: any) { setApiError(e?.detail ?? 'Failed to load.'); }
    ...
    const handleSceneSubmit = async (data: Omit<SceneCreate, 'project_id'>) => {
        setMutating(true);
        try {
            if (editingScene) { const u = await sceneService.update(editingScene.id, data as SceneUpdate); setScenes(p => p.map(s => s.id === u.id ? u : s).sort((a, b) => a.scene_order - b.scene_order)); notify('Scene updated!'); }
            else { const c = await sceneService.create({ ...data, project_id: id }); setScenes(p => [...p, c].sort((a, b) => a.scene_order - b.scene_order)); pickScene(c.id); notify('Scene created!'); }
    ...
    const handleDeleteScene = async () => {
        ...
        setMutating(true);
        try { await sceneService.delete(deletingSceneId); setScenes(p => p.filter(s => s.id !== deletingSceneId)); if (selectedSceneId === deletingSceneId) setSelectedSceneId(null); setDeletingSceneId(null); notify('Scene deleted.'); }
        catch { notify('Failed.', true); } finally { setMutating(false); }
    };
    ...
    const handleLocSubmit = async (data: any) => {
        ...
        try {
            if (editingLoc) {
                const u = await locationService.update(editingLoc.id, data);
                setLocations(p => p.map(l => l.id === u.id ? u : l));
                notify('Location updated!');
            }
            else {
                const c = await locationService.create({ ...data, project_id: id });
                setLocations(p => [...p, c]);
                notify('Location created!');
                // Auto-assign to current scene if open
                if (selectedSceneId) {
                    const u = await sceneService.update(selectedSceneId, { location_id: c.id });
    ...
    const handleDeleteLoc = async (lid: string) => {
        ...
        setMutating(true);
        try { await locationService.delete(lid); setLocations(p => p.filter(l => l.id !== lid)); notify('Location deleted.'); }
        catch (e: any) { notify(e.message, true); }
        finally { setMutating(false); }
    };

    const handleSceneStateSubmit = async (charId: string, payload: { age?: string; outfit?: string } | null) => {
        if (!castModalSceneId) return;
        ...
        try {
            ...
            // we have to cast nextStates to any, because character_states in DB is JSONB and in API it's typed. But TypeScript should be ok since we typed it as Record<string, {age, outfit}>
            const u = await sceneService.update(castModalSceneId, { character_states: nextStates as any });
    ...
    const handleAddShot = async () => {
        ...
        try {
            ...
            const c = await shotService.create({ scene_id: selectedSceneId, shot_order: existing.length + 1, target_duration: 5 });
    ...
    const handleSaveShot = async (shotId: string, payload: ShotUpdate) => {
        try {
            const u = await shotService.update(shotId, payload);
    ...
    const handleDeleteShot = async () => {
        ...
        setMutating(true);
        try {
            await shotService.delete(deletingShotId);
    ...
    const handleRender = async (shotId: string) => {
        try { await shotService.generate(shotId); notify('🎬 Render queued!'); }
    ...
    const handleCharSubmit = async (data: { name: string; personality_traits: string; outfit_description: string; actor_id: string }) => {
        ...
        try {
            if (editingChar) {
                const u = await characterService.update(editingChar.id, { name: data.name, personality_traits: data.personality_traits || null, outfit_description: data.outfit_description || null, actor_id: data.actor_id || null });
                setCharacters(p => p.map(c => c.id === u.id ? u : c));
                notify('Character updated!');
            } else {
                const c = await characterService.create({ name: data.name, personality_traits: data.personality_traits || null, outfit_description: data.outfit_description || null, actor_id: data.actor_id || null, project_id: id });
                setCharacters(p => [...p, c]);
    ...
    const handleDeleteChar = async () => {
        ...
        setMutating(true);
        try {
            await characterService.delete(deletingCharId);
    ...
    return (
        <div className="app-layout">
            ...
            <div className="main-content" style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden', background: '#f8fafc' }}>
                ...
                <div style={{ height: 52, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 20px', borderBottom: '1px solid var(--color-border)', background: 'var(--color-surface)', flexShrink: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        {project && <span style={{ fontSize: 10, color: 'var(--color-text-tertiary)', background: 'var(--color-surface-2)', padding: '2px 8px', borderRadius: 4 }}>{project.aspect_ratio} · {project.project_type?.replace('_', ' ')}</span>}
                    </div>
                ...
                {loading ? (
                    ...
                    <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
                            ...
                            {leftTab === 'scenes' && (
                                <>
                                    <div style={{ padding: '8px 10px', borderBottom: '1px solid var(--color-border)', display: 'flex', justifyContent: 'flex-end' }}>
                                        <button className="btn btn-primary" style={{ padding: '3px 10px', fontSize: 11 }} onClick={() => { setEditingScene(null); setSceneModalOpen(true); }}>+ Add</button>
                                    </div>
                                    <div style={{ flex: 1, overflowY: 'auto' }}>
                                        {scenes.length === 0 ? (
                                            <div style={{ padding: '32px 16px', textAlign: 'center', color: 'var(--color-text-tertiary)', fontSize: 11 }}><div style={{ fontSize: 24, marginBottom: 6 }}>🖋️</div>No scenes yet.</div>
                                        ) : scenes.map(scene => {
                                            const isActive = scene.id === selectedSceneId;
                                            return (
                                                <div key={scene.id} onClick={() => pickScene(scene.id)}
                                                    style={{ padding: '9px 12px', cursor: 'pointer', borderLeft: `3px solid ${isActive ? 'var(--color-accent)' : 'transparent'}`, background: isActive ? '#eff6ff' : 'transparent', transition: 'all 0.12s' }}>
                                                    <div style={{ display: 'flex', justifyContent: 'space-between', gap: 4 }}>
                                                        ...
                                                        <div style={{ display: 'flex', gap: 1, flexShrink: 0 }} onClick={e => e.stopPropagation()}>
                                                            <button style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 11, opacity: 0.5, padding: '1px 3px' }} onClick={() => { setEditingScene(scene); setSceneModalOpen(true); }}>✏️</button>
                                                            <button style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 11, opacity: 0.5, padding: '1px 3px', color: 'var(--color-danger)' }} onClick={() => setDeletingSceneId(scene.id)}>🗑</button>
                                                        </div>
                                                    </div>
                                                </div>
                                            );
                            ...
                            {selectedScene ? (
                                <>
                                    ...
                                    <div style={{ background: 'var(--color-surface)', borderBottom: '1px solid var(--color-border)', flexShrink: 0 }}>
                                        <div style={{ padding: '14px 24px 10px' }}>
                                            ...
                                            <div style={{ display: 'flex', gap: 12, fontSize: 11, color: '#94a3b8', fontFamily: 'Courier Prime,Courier New,monospace' }}>
                                                ...
                                                <span style={{ marginLeft: 'auto', color: isLoadingShots ? '#94a3b8' : 'var(--color-accent)' }}>{currentShots.length} shot{currentShots.length !== 1 ? 's' : ''}</span>
                                            </div>
                                        </div>
                                                    ...
                                                    {loc ? (
                                                        <>
                                                            ...
                                                            <div style={{ flex: 1, minWidth: 0 }}>
                                                                ...
                                                                {loc.lighting_hint && <div style={{ fontSize: 10, fontWeight: 700, color: '#ca8a04', marginTop: 4 }}>💡 Lighting: {loc.lighting_hint}</div>}
                                                            </div>
                                                        </>
                                                    ) : (
                                    ...
                                    <div style={{ flex: 1, overflowY: 'auto', background: '#fff' }}>
                                        ...
                                        {/* End of scene marker */}
                                        {currentShots.length > 0 && (
                                            <div style={{ padding: '24px 48px 40px', textAlign: 'center' }}>
                        ...
                        {/* ===== RIGHT: Shot Strip (thumbnails) ===== */}
                        <div style={{ width: 200, flexShrink: 0, borderLeft: '1px solid var(--color-border)', background: 'var(--color-surface)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                            <div style={{ padding: '10px 12px', borderBottom: '1px solid var(--color-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <span style={{ fontSize: 10, fontWeight: 800, color: 'var(--color-text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Storyboard</span>
                                {selectedScene && <button className="btn btn-primary" style={{ padding: '3px 8px', fontSize: 10 }} disabled={mutating} onClick={handleAddShot}>+ Shot</button>}
                            </div>
                            <div style={{ flex: 1, overflowY: 'auto', padding: '8px' }}>
                                {isLoadingShots ? (
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                                        {[1, 2, 3].map(i => <div key={i} className="skeleton" style={{ width: '100%', aspectRatio: '16/9', borderRadius: 6 }} />)}
                                    </div>
                                ) : (
                                    currentShots.map(shot => {
                                        const sc = statusStyle(shot.status);
                                        const parsed = parseShotScript(shot.sfx_prompt);
                                        const isAct = activeShot === shot.id;
                                        return (
                                            <div key={shot.id} onClick={() => { setActiveShot(shot.id); shotRefs.current[shot.id]?.scrollIntoView({ behavior: 'smooth', block: 'center' }); }}
                                                style={{ marginBottom: 8, cursor: 'pointer', borderRadius: 8, border: `2px solid ${isAct ? 'var(--color-accent)' : 'var(--color-border)'}`, overflow: 'hidden', transition: 'all 0.12s' }}>
                                                {/* Thumbnail */}
                                                <div style={{ aspectRatio: '16/9', background: 'linear-gradient(135deg,#1a1a2e,#16213e)', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
                                                    {shot.preview_image_url ? <img src={shot.preview_image_url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : (
                                                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.2)" strokeWidth="1.5"><polygon points="23 7 16 12 23 17 23 7" /><rect x="1" y="5" width="15" height="14" rx="2" /></svg>
                                                    )}
                                                    <div style={{ position: 'absolute', top: 4, left: 4, background: 'rgba(0,0,0,0.7)', color: '#fff', borderRadius: 4, padding: '1px 5px', fontSize: 9, fontWeight: 800 }}>#{shot.shot_order}</div>
                                                    ...
                                                    {shot.target_duration && <div style={{ position: 'absolute', bottom: 4, right: 4, background: 'rgba(0,0,0,0.6)', color: '#fff', borderRadius: 3, padding: '1px 5px', fontSize: 8 }}>{shot.target_duration}s</div>}
                                                </div>
                                                {/* Meta */}
                                                <div style={{ padding: '5px 7px', background: 'var(--color-surface)' }}>
                                                    <div style={{ fontSize: 9, color: 'var(--color-text-tertiary)', marginBottom: 1 }}>
                                                        {[shot.camera_angle, shot.motion_type].filter(Boolean).join(' · ') || 'No camera set'}
                                                    </div>
                                                    {parsed.action && <div style={{ fontSize: 9, color: 'var(--color-text-secondary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontStyle: 'italic' }}>{parsed.action}</div>}
                                                    {parsed.dialogue.length > 0 && <div style={{ fontSize: 9, color: '#94a3b8', marginTop: 1 }}>💬 {parsed.dialogue.length} line{parsed.dialogue.length > 1 ? 's' : ''}</div>}
                                                </div>
                                            </div>
                                        );
                                    })
                                )}
                                {currentShots.length === 0 && !isLoadingShots && selectedScene && (
                                    <div style={{ textAlign: 'center', padding: '32px 8px', color: '#94a3b8', fontSize: 10 }}>No shots yet</div>
                                )}
                            </div>
                        </div>
                    </div>
            ...
            <Modal open={editProjectOpen} onClose={() => setEditProjectOpen(false)} title="Edit Project">
                {project && <EditProjectForm project={project} loading={mutating} onSubmit={async p => { setMutating(true); try { const u = await projectService.update(project.id, p); setProject(u); setEditProjectOpen(false); notify('Saved!'); } catch { notify('Failed.', true); } finally { setMutating(false); } }} onClose={() => setEditProjectOpen(false)} />}
            </Modal>
...
function EditProjectForm({ project, loading, onSubmit, onClose }: { project: ProjectRead; loading: boolean; onSubmit: (d: ProjectUpdate) => void; onClose: () => void; }) {
    const [name, setName] = useState(project.name); const [logline, setLogline] = useState(project.logline ?? ''); const [ar, setAr] = useState(project.aspect_ratio); const [status, setStatus] = useState(project.status);
    return (
        <form onSubmit={e => { e.preventDefault(); onSubmit({ name, logline: logline || undefined, aspect_ratio: ar, status }); }}>
            <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 14 }}>
