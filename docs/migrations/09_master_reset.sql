-- 1. DELETE DUPLICATE USERS! (Keep only the newest one per email)
-- This fixes the bug where logging in multiple times created multiple shadow profiles.
...
-- 2. WIPE TENANT MEMBERS TO CLEAN UP GHOSTS
TRUNCATE TABLE public.tenant_members;

-- 3. RE-INSERT REAL OWNERS ONLY
...
-- 4. DROP EVERY EXISTING POLICY SO THERE ARE NO CONFLICTS OR LEAKS
...
-- 6. SECURITY DEFINER FUNCTIONS (SAFE FROM LOOPS)
...
-- 10. CREATE DIAGNOSTIC ENDPOINT (So I can help check the final status)
CREATE OR REPLACE FUNCTION public.debug_dump()
RETURNS json
LANGUAGE sql
SECURITY DEFINER
SET search_path = public
AS $$
  SELECT json_build_object(
    ...
    'tenants', (SELECT json_agg(row_to_json(t)) FROM public.tenants t),
    'tenant_members', (SELECT json_agg(row_to_json(tm)) FROM public.tenant_members tm)
  );
$$;
GRANT EXECUTE ON FUNCTION public.debug_dump() TO anon, authenticated, service_role;
