...
CREATE TABLE public.characters (
  ...
  approved_reference_image_url text,
  consistency_locked boolean DEFAULT false,
  reference_generation_id uuid,
  CONSTRAINT characters_pkey PRIMARY KEY (id),
  CONSTRAINT characters_reference_generation_id_fkey FOREIGN KEY (reference_generation_id) REFERENCES public.shot_generations(id),
...
CREATE TABLE public.render_jobs (
  ...
  ai_provider text,
  ai_model text,
  credit_cost numeric DEFAULT 0.00,
  triggered_by text,
  parent_job_id uuid,
  completed_at timestamp without time zone,
  provider character varying DEFAULT 'runway'::character varying,
  ...
  CONSTRAINT render_jobs_parent_job_id_fkey FOREIGN KEY (parent_job_id) REFERENCES public.render_jobs(id),
...
CREATE TABLE public.shot_generations (
  ...
  created_at timestamp without time zone DEFAULT now(),
  updated_at timestamp without time zone DEFAULT now(),
  generation_type text DEFAULT 'video'::text CHECK (generation_type = ANY (ARRAY['image'::text, 'video'::text])),
  user_rating smallint DEFAULT 0 CHECK (user_rating = ANY (ARRAY['-1'::integer, 0, 1])),
...
CREATE TABLE public.shots (
  ...
  ai_prompt text,
  video_url text,
  continuity_mode text DEFAULT 'auto'::text,
  visual_prompt text,
  image_prompt text,
  video_prompt text,
  approved_image_url text,
  image_generation_id uuid,
  clip_parts jsonb DEFAULT '[]'::jsonb,
  calculated_duration numeric,
  CONSTRAINT shots_pkey PRIMARY KEY (id),
  CONSTRAINT shots_image_generation_id_fkey FOREIGN KEY (image_generation_id) REFERENCES public.shot_generations(id),
